        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\Action::make('push_notification')
                        ->label('Push Alert')
                        ->icon('heroicon-o-bell-alert')
                        ->color('warning')
                        ->requiresConfirmation()
                        ->modalHeading('Send Web Push Notification')
                        ->modalDescription('This will send a real-time browser push notification to ALL subscribed users on their phones & desktops.')
                        ->hidden(fn (Post $record) => $record->status !== 'published')
                        ->action(function (Post $record) {
                            $frontendUrl = env('FRONTEND_URL', 'https://newsthetruth.com');
                            $articleUrl  = $frontendUrl . '/news/' . $record->slug;
                            $imageUrl    = $record->thumbnailMedia
                                ? rtrim(config('app.url'), '/') . '/storage/' . ltrim($record->thumbnailMedia->url, '/')
                                : null;
                            $excerpt = $record->excerpt
                                ? \Illuminate\Support\Str::limit(strip_tags($record->excerpt), 100)
                                : \Illuminate\Support\Str::limit(strip_tags($record->description), 100);

                            $service = new \App\Services\OneSignalService();
                            $sent    = $service->sendToAll($record->title, $excerpt, $articleUrl, $imageUrl);

                            if ($sent) {
                                \Filament\Notifications\Notification::make()
                                    ->title('🔔 Push Notification Sent!')
                                    ->body('Breaking news alert delivered to all subscribed devices.')
                                    ->success()
                                    ->send();
                            } else {
                                \Filament\Notifications\Notification::make()
                                    ->title('Push Notification Failed')
                                    ->body('Check your ONESIGNAL_APP_ID and ONESIGNAL_REST_API_KEY in the .env file.')
                                    ->danger()
                                    ->send();
                            }
                        }),
