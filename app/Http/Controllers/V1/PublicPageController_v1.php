...
class PublicPageController_v1 extends Controller
{
    ...
    public function index($x = '')
    {
        ...
        //echo date('H:i:s').'<br>';
        $data['top_post'] = Post::where('top_post', 1)->where('status', 'published')->where('visibility', 'public')->where('ignore_top_scheduling', '1')->orWhere([['top_post_to_time', '>', date('H:i:s')], ['top_post_form_time', '<', date('H:i:s')], ['post_publish_time', '<', date('Y-m-d H:i:s')]])->limit(5)->offset(0)->orderBy('created_at', 'DESC')->get();
        //dd( $data );
        //echo 'top_post'.date('H:i:s').'<br>';
        $data['the_latest'] = Post::where('status', 'published')->where('visibility', 'public')->limit(10)->offset(0)->orderBy('created_at', 'DESC')->get();
        //echo 'the_latest'.date('H:i:s').'<br>';
        ...
        // echo 'just_ins'.date('H:i:s').'<br>';
        $categories = ['15', '12', '17', '21', '19', '22'];
        ...
        //echo 'the_exclusive'.date('H:i:s').'<br>';
    ...
    public function post($x = [])
    {
        ...
        $body_classes = 'singular post-single page-template-default post no-sidebar';
        return view('public-pages.post')->with(['the_post' => $x, 'seo_data' => $seo_data, 'meta' => $meta, 'top_post' => $top_post,  'just_ins' => $just_ins, 'the_latest' => $the_latest, 'similars' => $similars, 'body_classes' => $body_classes, 'settings' => $this->settings, 'youtube_data' => $youtube_data]);
    }
    ...
    public function page($x = '')
    {
        ...
        $seo_data['meta']['type'] = 'article';
        $seo_data['meta']['modified_time'] = $x->updated_at;
        return view('public-pages.page')->with(['the_page' => $x, 'body_classes' => $body_classes, 'settings' => $this->settings, 'seo_data' => $seo_data]);
    }

    public function category($x = '')
    {
        /*$the_archive = Post::where('status', 'published')->whereHas( 'categories', function( $query ) use( $x ){
            return $query->whereHas('cat_data', function($q) use($x){
                return $q->where('category_id', $x->id);
            });
        })->paginate(10);*/
        ...
        return view('public-pages.category')->with(['the_archive' => $x, 'posts' => $the_archive, 'seo_data' => $seo_data, 'top_post' => $top_post, 'the_latest' => $the_latest, 'categories' => $categories, 'cat_data' => $x, 'body_classes' => $body_classes, 'settings' => $this->settings]);
    }
