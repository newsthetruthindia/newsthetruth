            ...
            Section::make('Media')
                ->schema([
                    Select::make('thumbnail')
                        ...
                        ->preload()
                        ->allowHtml()
                        ->getOptionLabelFromRecordUsing(fn ($record) => "<div style='display:flex; align-items:center; gap:8px;'><img src='".asset('storage/' . ltrim($record->url, '/'))."' style='height:35px; width:50px; object-fit:cover; border-radius:4px;'> <span>" . basename($record->url) . "</span></div>")
                        ->columnSpanFull(),
