...
<section class="post-single-content hide-mobile">
	<div class="container-fluid">
		<div class="row bg-white">
			<div class="col-lg-3 col-md-3 col-sm-3 col-12 latest-left-col">
			    ...
			    @if( !empty( $the_latest ) && count( $the_latest ) > 0 )
			        <div class="row article">
						<div class="col-md-12 video">
						    @if( !empty( \App\Http\Controllers\SiteSettingsController::check('post_left') ) )
    	                        @php
    	                            $post_leftlogo = getAttachmentById(\App\Http\Controllers\SiteSettingsController::check('post_left')->description);
    	                        @endphp
    	                        @if( !empty( $logo->url ) )
    	                            <a href="{{ !empty(\App\Http\Controllers\SiteSettingsController::check('post_left_add_url')->description)?\App\Http\Controllers\SiteSettingsController::check('post_left_add_url')->description:'/' }}" target="_blank">
    	                                <img class="main-logo" src="{{ url($post_leftlogo->url) }}" alt="" style="width: 100%;"/>
    	                            </a>
    	                        @endif
    	                    @endif
						</div>
						...
						<div class="row article">
							<div class="col-md-12 video">
							    <div class="card">
    								...
    								<h5><a href="{{ url($latest->slug) }}">{{ substr($latest->title,0,70); }}...</a></h5>
								</div>
							</div>
						</div>
					@endforeach
			...
			<div class="col-lg-6 col-md-6 col-sm-6 col-12 news-section">
                            ...
                            <ul class="tags" style="margin-top: 20px;">
                                @foreach( $the_post->tags as $tag )
                                    @if( !empty( $tag->tag_data ) )
                                        <li><a class="label tag" href="{{ url( $tag->tag_data->slug ) }}">{{ $tag->tag_data->title }} </a></li>
                                    @endif
                                @endforeach
                            </ul>
                        </div>
				...
				<div class="row">
				    <div class="col-md-12">
				        <h2 class="bg-white text-dark">{{ $the_post->title }}</h2>
				    </div>
				</div>
				<div class="row news-info">
					<div class="col-lg-12">
						{{ !empty($meta['credit']) ? $meta['credit'] : (!empty( $the_post->user ) ? $the_post->user->firstname.' '.$the_post->user->lastname:'Staff Reporter')}}
						&nbsp;&nbsp;&nbsp;
						<!--<i class="fa fa-map-marker"></i> Place: {{ !empty($meta['place']) ? $meta['place'] : 'Kolkata'}}
						&nbsp;&nbsp;&nbsp;-->
						<?php date_default_timezone_set('Asia/Kolkata'); ?>
						{{ date('M d, Y h:i a', strtotime($the_post->created_at.'+330 minutes') ) }}
					</div>
				</div>
				@if( !empty( $the_post->thumbnails ) )
					<div class="row thumbnails">
						<div class="col-lg-12">
							<img src="{{ url($the_post->thumbnails->url) }}"  srcset="{{ get_image_srcset($the_post->thumbnails->id)}}"/>
				...
				@if( !empty( $the_post->audio_clip_url ) )
					<div class="row text-center">
						<audio controls style="height:10vh;">
				...
				@endif
				@if( !empty( $the_post->gallery ) && count( $the_post->gallery ) > 0 )
						...
						<div class="col-lg-12 text-center">
							@foreach($the_post->gallery as $e_t_g_k => $e_t_g_v )
								...
								@if( !empty( $e_t_g_image->url ) )
									<img src="{{ url($e_t_g_image->url) }}" alt="" srcset="{{ get_image_srcset($e_t_g_v->media_id)}}"/>
								@endif
							@endforeach
						</div>
					</div>
				@endif
				<div class="row news-info" style="margin-top:20px;">
					<div class="col-lg-12">
						<i class="fa fa-user"></i> Reported By: {{ !empty($meta['credit']) ? $meta['credit'] : (!empty( $the_post->user ) ? $the_post->user->firstname.' '.$the_post->user->lastname:'Staff Reporter')}}
						&nbsp;&nbsp;&nbsp;
						<i class="fa fa-map-marker"></i> Place: {{ !empty($meta['place']) ? $meta['place'] : 'Kolkata'}}
						&nbsp;&nbsp;&nbsp;
						<?php date_default_timezone_set('Asia/Kolkata'); ?>
						<i class="fa fa-clock-o"></i> {{ date('M d, Y h:i a', strtotime($the_post->created_at.'+330 minutes') ) }}
					</div>
				</div>
					...
					<div class="row social-share text-center">
						...
						<div class="col-lg-12">
							<ul>
								...
								<li>
									<a href='#' id="ntt_share" data-url='{{ url($the_post->slug) }}' data-toggle="tooltip" title="Share">
										<image src="{{ asset('public/img/social/share_icon.svg') }}"/>
									</a>
								</li>
							</ul>
						</div>
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-12 add-section">
				<div class="add">
					@if( !empty( $the_post->metas()->where('key', 'add_url')->first() ))
						...
						@else
							...
							@endif
						@endif
					@elseif( !empty( $the_post->categories[0]) && !empty( $the_post->categories[0]->cat_data->metas()->where('key', 'add_url')->first() ))
						<?php
							$cat_addurl_items = $the_post->categories[0]->cat_data->metas()->where('key', 'add_url')->first()->description;
						?>
						@if( !empty($cat_addurl_items ) )
		...
		<div class="row bg-white related-news">
		    ...
		    <div class="col-md-12 text-center">
		        <div class="row">
		            @if( !empty( $similars ) && count( $similars ) > 0 )
    					@foreach( $similars as $key => $latest )
    						<div class="col-md-2">
    							<div class="card">
        								...
        								<p><a href="{{ url($latest->slug) }}">{{ substr($latest->title,0,45); }}...</a></p>
        							</div>
		...
		@php
            $enable_just_in = true;
        @endphp
        @if( !empty( \App\Http\Controllers\SiteSettingsController::check('enable_just_in') ) )
            @php
                $enable_just_in =false;// \App\Http\Controllers\SiteSettingsController::check('enable_just_in')->description;
            @endphp
        @endif
        @if( !empty( $just_in ) && $enable_just_in)
		<div class="row bg-white just-ins">
		    <div class="col-md-12 text-center">
		        <h2> Just In </h2>
		    </div>
		    <div class="col-md-12 text-center">
		        <div class="row">
		            @if( !empty( $just_ins ) && count( $just_ins ) > 0 )
    					@foreach( $just_ins as $key => $latest )
    						<div class="col-md-2">
    							<div class="card">
        								...
        								<p><a href="{{ url($latest->slug) }}">{{ substr($latest->excerpt,0,45); }}...</a></p>
...
<section class="post-single-content hide-desktop">
	<div class="container-fluid">
		<div class="row bg-white">
			<div class="col-lg-9 news-section">
				...
				<div class="row news-info">
					<div class="col-lg-12">
						...
						&nbsp;&nbsp;&nbsp;
						<?php date_default_timezone_set('Asia/Kolkata'); ?>
						<i class="fa fa-clock-o"></i> {{ date('d-m-Y h:i:s a', strtotime($the_post->created_at.'+330 minutes') ) }}
					</div>
				</div>
				@if( !empty( $the_post->thumbnails ) )
					<div class="row thumbnails">
...
<section class="home-archive hide-desktop">
	<div class="container-fluid">
		<div class="row heading">
			<div class="col-md-12">
				<h3> Top Posts </h3>
...
<section class="home-archive hide">
	<div class="container-fluid">
...
<section class="home-archive hide">
	<div class="container-fluid youtube-reel">
		<div class="row">
			@if( !empty( $youtube_data['items'] ) && count( $youtube_data['items'] ) > 0 )
				@foreach( $youtube_data['items'] as $key => $value )
					<div class="col-md-3 item">
						<iframe width="270" height="150" src="https://www.youtube.com/embed/{{ $value['id']['videoId'] }}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
						@if( $value['snippet']['title'] )
						    <div>{{ trim(addslashes($value['snippet']['title'])) }}
						    </div>
						@endif
					</div>
				@endforeach
...
$(document).ready(function(){
    ...
    $(window).on('scroll', function(){
        ...
        if( $('header').hasClass('fixed') ){
            $('.row.heading').eq(0).addClass('fixed');
        }else{
            $('.row.heading').removeClass('fixed');
        }
    })
})
